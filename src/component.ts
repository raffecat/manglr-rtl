...
import { Cell, Scope, SpawnCtx, VNode } from './types';

// sc { tpl, ofs, syms, fragment, spawn_children }

// -+-+-+-+-+-+-+-+-+ Component Spawn -+-+-+-+-+-+-+-+-+

export function create_component(sc:SpawnCtx, parent:VNode, outer_scope:Scope): void {
  ...
  const cssm = sc.syms[sc.tpl[tpl_ofs]!] as string; // tpl[0] symbol for CSS namespacing.
  ...
  const new_scope:Scope = {
    ...
    cssm: cssm,
    c_tpl: c_tpl,
    c_locals: outer_scope.locals,
    c_cssm: outer_scope.cssm,
    d_list: outer_scope.d_list
  };
