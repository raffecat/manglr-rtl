...
import { DList, DListArg, DListFunc } from "./types";
...
export function d_list_add(d_list:DList, func:DListFunc, arg:DListArg): void {
  d_list['push'](func, arg);
}

export function run_d_list(d_list:DList, in_destroy:boolean): void {
  ...
  for (let i=0; i < d_list['length']; i += 2) {
    (d_list[i] as DListFunc)((d_list[i+1] as any), in_destroy);
  }
  d_list['length'] = 0;
}
